...
// key is plane name
let layerPlans = {
  "cutout": (keymap, plane) => difference(baseKeyboard(keymap), keyCutouts(keymap, plane)),
...
function truncateOutput(layer, keymap, argv) {
  ...
  console.log("Layer Max Width:", maxWidth, " Layer Max Height:", maxHeight)
  ...
  console.log("Truncating output at positions:", startX, startY, endX, endY)

  return intersection(
    layer,
    square([endX - startX, endY - startY]).translate([startX, startY])
  )
}
...
function layerPlan(keymap, argv) {
  ...
  let layer = layerPlans[argv.layer](keymap, argv.layer)

  if (argv.truncate)
    return truncateOutput(layer, keymap, argv)
