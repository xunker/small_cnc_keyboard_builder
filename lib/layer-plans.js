...
const { rowLength, isFirstRow, isLastRow, firstKey, lastKey, rangeIntersection, maxLayerWidth, maxLayerLength } = require('./misc')
...
function stabilizers(keyobj, width, length, corner_r) {
  ...
  signs.forEach(function (sign, index) {
    objects.push(
      rounded_rect(width, length, { r: corner_r })
        .translate([(config.stabilizerSpacingForWidth(keyobj.width) / 2) * sign, (((config.dimensions.switch_unit_l - length) / 2) + ((config.dimensions.switch_cutout_l - length)/2)) - config.dimensions.stabilizer_y_offset])
...
function cutout(keyObj, main_w, main_l, main_corner_r, stabilizer_main_width, stabilizer_main_length, stabilizer_main_corner_r, include_stabilizers = false) {

  var obj = new Array()

  obj.push(
    rounded_rect(main_w, main_l, { r: main_corner_r }).translate([0, (config.dimensions.switch_unit_l - main_l) / 2]).translate([(config.dimensions.switch_unit_w - main_w) / 2, 0])
  )

  if (include_stabilizers) {
    obj.push(
      stabilizers(keyObj, stabilizer_main_width, stabilizer_main_length, stabilizer_main_corner_r)
    )
  }
...
function caseCutouts(keymap) {
  ...
  if (config.dimensions.caseWall.thickness) {
    verbose(`caseCutouts: Using config.dimensions.caseWall.thickness: ${config.dimensions.caseWall.thickness}`)
    inset_x = config.dimensions.caseWall.thickness
    inset_y = config.dimensions.caseWall.thickness
  } else {
    verbose(`caseCutouts: Using defaults: inset_x: ${inset_x}, inset_y: ${inset_y}`)
  }
...
let layerPlans = {
  ...
  "case": function (keymap) {
    ...
    if (sr.length) {
      verbose('Adding screw holes')
      return difference(baseKeyboard(keymap), union(caseCutouts(keymap), sr))
    }

    return difference(baseKeyboard(keymap), caseCutouts(keymap))
  },
  "bottom": function(keymap) {
    let sr = screwHoles(keymap)
    verbose(`Screw holes: ${sr.length}`)
    if (sr.length) {
      verbose('Adding screw holes')
      return difference(baseKeyboard(keymap), union(sr))
    }
...
function truncateOutput(layer, keymap, argv) {
  if (renderRows.length)
    fatalError("--rows argument is not compatible with --truncate")
  ...
  let maxLength = maxLayerLength(keymap) * config.dimensions.switch_unit_w

  if ((config.dimensions.extendFootprint.left) && (firstKey(keymap[0]).isFirstVisible)) {
    verbose(`truncateOutput: extendFootprint.left: Adjusting width by ${config.dimensions.extendFootprint.left}`)
    maxWidth += config.dimensions.extendFootprint.left
  }

  if ((config.dimensions.extendFootprint.right) && (lastKey(keymap[0]).isLastVisible)) {
    verbose(`truncateOutput: extendFootprint.right: Adjusting width by ${config.dimensions.extendFootprint.right}`)
    maxWidth += config.dimensions.extendFootprint.right
  }

  if (config.dimensions.extendFootprint.top) {
    verbose(`truncateOutput: extendFootprint.top: Adjusting length by ${config.dimensions.extendFootprint.top}`)
    maxLength += config.dimensions.extendFootprint.top
  }

  if (config.dimensions.extendFootprint.bottom) {
    verbose(`truncateOutput: extendFootprint.bottom: Adjusting length by ${config.dimensions.extendFootprint.bottom}`)
    maxLength += config.dimensions.extendFootprint.bottom
  }

  if ((config.dimensions.kerf.footprint) && (config.dimensions.kerf.footprint > 0.0)) {
    verbose(`truncateOutput: dimensions.kerf.footprint: Adjusting length by ${config.dimensions.kerf.footprint}`)
    maxWidth += config.dimensions.kerf.footprint * 2
    maxLength += config.dimensions.kerf.footprint * 2
  }

  info(`Layer Max Width: ${maxWidth}, Layer Max Length: ${maxLength}`)
  ...
  if ((config.dimensions.extendFootprint.left) && (firstKey(keymap[0]).isFirstVisible)) {
    verbose(`truncateOutput: extendFootprint.left: Adjusting minRowX by -${config.dimensions.extendFootprint.left}`)
    minRowX -= config.dimensions.extendFootprint.left
  }

  if (config.dimensions.kerf.footprint) {
    verbose(`truncateOutput: dimensions.kerf.footprint: Adjusting minRowX by -${config.dimensions.kerf.footprint}`)
    minRowX -= config.dimensions.kerf.footprint
  }


  /* same problem for Y axis, related to extendfootprint and kerf */
  let minRowY = 0

  if (config.dimensions.extendFootprint.top) {
    verbose(`truncateOutput: extendFootprint.top: Adjusting minRowY by -${config.dimensions.extendFootprint.top}`)
    minRowY -= config.dimensions.extendFootprint.top
  }

  if (config.dimensions.kerf.footprint) {
    verbose(`truncateOutput: dimensions.kerf.footprint: Adjusting minRowY by -${config.dimensions.kerf.footprint}`)
    minRowY -= config.dimensions.kerf.footprint
  }

  verbose(`minRowX: ${minRowX}, minRowY: ${minRowY}`)

  /* end of section/extend/kerf shift fix */

  startX = maxWidth * (startX / 100)
  endX = maxWidth * (endX / 100)
  startY = maxLength * (startY / 100)
  endY = maxLength * (endY / 100)
  ...
  // return difference(
    layer,
    square([endX - startX, endY - startY]).translate([minRowX, minRowY]).translate([startX, startY])
  )
}
...
function layerPlan(keymap, argv) {
  ...
  let renderedLayer = layerPlanFunction(keymap)

  if (argv.postProcessing) {
    verbose(`Applying post-processing functions: ${argv.postProcessing}`)

    if (typeof (argv.postProcessing) == 'object') {
      argv.postProcessing.foreach((ppFile) => {
        verbose(ppFile)
        renderedLayer = require(`${process.cwd()}/${ppFile}`)(renderedLayer, keymap, config)
      })
    } else {
      renderedLayer = require(`${process.cwd()}/${argv.postProcessing}`)(renderedLayer, keymap, config)
    }
  }

  if (argv.truncate)
    renderedLayer = truncateOutput(renderedLayer, keymap, argv)
