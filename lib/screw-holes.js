...
const defaultScrewInset = defaultScrewHoleSize
...
function screwInset() {
  let d = config.dimensions.screws.inset;
  if (typeof (d) == 'undefined') {
    warn(`config.dimensions.screws.inset is not defined. Using default of ${defaultScrewInset}`)
    return defaultScrewInset
  }
...
function screwHole(location, keyObj, edges = {}) {
  ...
  const inset = screwInset();
  verbose(`screwInset: ${inset}`)
