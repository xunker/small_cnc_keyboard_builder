...
const defaultScrewHoleSize = 1.75

function screwDiameter() {
  let d = dimensions.screws.diameter;
  if (!d) {
    warn(`dimensions.screws.diameter is not defined. Using default of ${defaultScrewHoleSize}`)
    return defaultScrewHoleSize
  }

  if (dimensions.kerf.screws) {
    verbose(`Applying screw kerf of ${dimensions.kerf.screws}`)
    d -= dimensions.kerf.screws
    if (d < 0.5) {
      warn(`Screw diameter of ${d} with kerf of ${dimensions.kerf.screws} is less than minimum allowed diameter of 0.5; setting screw diameter to 0.5`)
      return 0.5
    }
  }
  return d
...
function screwHole(location, keyObj, edges = {}) {
  ...
  if ([0, 1, 2].indexOf(location) > -1) {
    // top
    // y_offset = (keyObj.height * dimensions.switch_unit_l) - (edges["top"] ? inset : 0)
    y_offset = (keyObj.height * dimensions.switch_unit_l) - ((edges["top"] && dimensions.extendFootprint.top < screwDiameter()) ? inset : 0)
  }

  if ([6, 7, 8].indexOf(location) > -1) {
    // bottom
    // y_offset = (edges["bottom"] ? inset : 0)
    y_offset = ((edges["bottom"] && dimensions.extendFootprint.bottom < screwDiameter()) ? inset : 0)
  }

  if ([0, 3, 6].indexOf(location) > -1) {
    // left
    x_offset = ((edges["left"] && (dimensions.extendFootprint.left < screwDiameter() || !keyObj.isFirstVisible)) ? inset : 0)
  }

  if ([2, 5, 8].indexOf(location) > -1) {
    // right
    // x_offset = (keyObj.width * dimensions.switch_unit_w) - (edges["right"] ? inset : 0)
    x_offset = (keyObj.width * dimensions.switch_unit_w) - ((edges["right"] && (dimensions.extendFootprint.right < screwDiameter() || !keyObj.isLastVisible)) ? inset : 0)
  }

  if ([1, 7].indexOf(location) > -1) {
    // middle top/bottom
    x_offset = (keyObj.width * dimensions.switch_unit_w) / 2
  }

  if ([3, 5].indexOf(location) > -1) {
    // middle left/right
    y_offset = (keyObj.height * dimensions.switch_unit_l) / 2
  }
...
function screwHoles(keymap) {
  verbose(`Building screw holes. Screw diameter: ${dimensions.screws.diameter}`)
  /*
      ...
      for (let screwPosition = 0; screwPosition <= 9; screwPosition++) {
        ...
        if ((keyObj.labels[screwPosition]) && (keyObj.labels[screwPosition].match('SCREW'))) {
          ...
          holes.push(
            ...
            ).translate([keyObj.x * dimensions.switch_unit_w, row_y_offset])
          )
        }
      }
    }
