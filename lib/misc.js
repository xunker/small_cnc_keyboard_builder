...
function rowLength(rowData) {
  return (lastKey(rowData).width + lastKey(rowData).x) - firstKey(rowData).x;
}
...
function maxLayerLength(keymap) {
  return keymap.length
}

module.exports = {
  rowLength, isFirstRow, isLastRow, firstKey, lastKey, rangeIntersection, maxLayerWidth, maxLayerLength
}
